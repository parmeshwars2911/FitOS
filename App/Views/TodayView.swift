...
struct TodayView: View {
    ...
    private var priorityMuscles: [MuscleState] {
        Array(store.trainingState.muscles.prefix(6))
    }
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("Train what needs attention.")
                .font(.title2.bold())
    ...
    private var generatorSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Button {
                store.generateWorkout(durationMinutes: durationMinutes)
            } label: {
